...
public class EntityCollectionSettersTest : TestBase
{
    ...
    {
        var hashSet = new HashSet<Entity<ChildEntity>>
        {
            ChildEntity.Create(1),
            ChildEntity.Create(2)
        }.Traverse();
    ...
    public void Test002_When_setting_a_entities_hashset_with_an_invalid_entity_then_the_entity_is_invalid()
    {
        ...
        {
            ChildEntity.Create(1),
            Entity<ChildEntity>.Invalid(new Error("Error"))
        }.Traverse();
    ...
    {
        ImmutableList<Entity<ChildEntity>> subEntities = ImmutableList<Entity<ChildEntity>>.Empty;
        subEntities = subEntities.Add(ChildEntity.Create(1));
        subEntities = subEntities.Add(ChildEntity.Create(2));

        var l = new HashSet<Entity<ChildEntity>>
        {
            ChildEntity.Create(1),
            ChildEntity.Create(2)
        };
    ...
    public void Test020_When_setting_a_collection_of_entities_in_an_invalid_entity_then_the_entity_stays_invalid()
    {
        ...
        var sut = Entity<MainEntityAsRecord>.Invalid(new Error("Invalid entity"));
        sut = sut.SetCollection(subEntities, static (e, l) => e with { ImmutableEntityList = l });
        var t = ChildEntity.Create(5);
    ...
    {
        var sut = MainEntityAsRecord.Create();
        ImmutableList<Entity<ChildEntity>> subEntities = null!;
        sut = sut.SetCollection(subEntities, static (e, l) => e with { ImmutableEntityList = l });
    ...
    {
        ImmutableList<Entity<ChildEntity>> entities = null!;
    ...
    {
        ImmutableDictionary<int, Entity<ChildEntity>> subEntities = ImmutableDictionary<int, Entity<ChildEntity>>.Empty;
        subEntities = subEntities.Add(1, ChildEntity.Create(1));
        subEntities = subEntities.Add(2, ChildEntity.Create(2));
    ...
    {
        var sut = MainEntityAsRecord.Create();
        ImmutableDictionary<int, Entity<ChildEntity>> subEntities = null!;
        sut = sut.SetDictionary(subEntities, static (e, l) => e with { ImmutableEntityDictionary = l });
